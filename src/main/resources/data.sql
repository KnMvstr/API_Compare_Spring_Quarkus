...
SELECT name, LOWER(name) AS slug
FROM (VALUES ('Toyota'), ('Honda'), ('Ford'), ('Chevrolet'), ('BMW'),
             ('Mercedes'), ('Audi'), ('Tesla'), ('Nissan'), ('Hyundai'),
             ('Kia'), ('Porsche'), ('Jaguar'), ('Lexus'), ('Land Rover'),
             ('Fiat'), ('Mazda'), ('Subaru')) AS v(name)
WHERE NOT EXISTS (
...
SELECT name, LOWER(name) AS slug
FROM (VALUES ('Sedan'), ('SUV'), ('Hatchback'), ('Convertible'), ('Coupe')) AS v(name)
WHERE NOT EXISTS (
...
SELECT * FROM (VALUES ('Maroon', '128,0,0', '#800000'), ('Lime Green', '50,205,50', '#32CD32'),
...
-- Insert engine into the model table
     ValidEngines AS (SELECT model,
                             ...
                             CASE
                                 ...
                                 WHEN model IN ('Prius', 'Fit', 'Malibu', 'Q5', 'X5', 'Outback', 'Forester', 'Crosstrek', 'Impreza',
                                                'Legacy') THEN 5
